...
if uploaded:
    ...
    if 'DI' not in df.columns:
        ...
        if st.button("🚀 Retrieve Keywords from OpenAlex"):
            ...
            # Separate failed DOIs
            failed_df = df[df['OpenAlex_KW'].str.contains("ERROR", na=False)]

            st.markdown("### 📄 Download Enriched Dataset")
            st.markdown("The enriched dataset includes a new `OpenAlex_KW` column. Choose your format:")
            ...
            if not failed_df.empty:
                st.warning(f"⚠️ {len(failed_df)} DOIs failed. A separate file is available for review.")
                st.markdown(generate_csv_download(failed_df, filename="Failed_DOIs.csv"), unsafe_allow_html=True)
