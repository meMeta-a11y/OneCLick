# streamlit_app.py

import streamlit as st
import pandas as pd
import requests
import base64
import time
import logging
import numpy as np
from io import BytesIO, StringIO

# ---------------- Streamlit Config ----------------
st.set_page_config(page_title="OneCLick Keyword Retriever", layout="centered")
st.title("🚀 OneCLick Keyword Retriever")

uploaded = st.file_uploader("📁 Upload Excel (.xlsx) with 'DI' (DOI)", type=["xlsx"])

# ---------------- Logging ----------------
logging.basicConfig(filename="error_log.txt", level=logging.INFO, format="%(asctime)s - %(message)s")

# ---------------- Fetch Keywords with Backoff ----------------
def fetch_openalex_keywords(doi, max_retries=3, backoff_factor=2):
    """Fetch OpenAlex keywords for a DOI with retry, backoff, and error logging."""
    url = f"https://api.openalex.org/works/doi:{doi}"
    retries = 0

    while retries < max_retries:
        try:
            r = requests.get(url, timeout=10)

            if r.status_code == 200:
                data = r.json()
                keywords = data.get("keywords", [])

                # Explicitly flag missing keywords
                if not keywords:
                    logging.warning(f"No keywords found for DOI {doi}")
                    return "[WARNING:NoKeywords]"

                return '; '.join(sorted(set(k['display_name'].strip() for k in keywords)))

            elif r.status_code == 429:  # Too many requests
                wait_time = int(r.headers.get("Retry-After", 2 ** retries))
                logging.warning(f"Rate limited for DOI {doi}. Retrying in {wait_time}s.")
                time.sleep(wait_time)

            else:
                logging.error(f"Failed DOI {doi} with status {r.status_code}")
                return f"[ERROR:{r.status_code}]"

        except requests.exceptions.Timeout:
            logging.error(f"Timeout error for DOI {doi}")
            return "[ERROR:Timeout]"

        except Exception as e:
            logging.error(f"Unexpected error for DOI {doi}: {str(e)}")
            return f"[ERROR:{str(e)}]"

        retries += 1

    return "[ERROR:MaxRetries]"

# ---------------- File Download Helpers ----------------
def generate_excel_download(df, filename="openalex_keywords.xlsx"):
    output = BytesIO()
    df.to_excel(output, index=False, engine='openpyxl')
    b64 = base64.b64encode(output.getvalue()).decode()
    return f'<a href="data:application/octet-stream;base64,{b64}" download="{filename}">📥 Download {filename}</a>'

def generate_csv_download(df, filename="openalex_keywords.csv"):
    output = StringIO()
    df.to_csv(output, index=False, encoding='utf-8-sig')
    b64 = base64.b64encode(output.getvalue().encode()).decode()
    return f'<a href="data:file/csv;base64,{b64}" download="{filename}">📥 Download {filename}</a>'

# ---------------- Main App Flow ----------------
if uploaded:
    df = pd.read_excel(uploaded, engine='openpyxl')

    if 'DI' not in df.columns:
        st.error("❌ Column 'DI' (DOI) is required.")
    else:
        df['OpenAlex_KW'] = ""

        if st.button("🚀 Retrieve Keywords from OpenAlex"):
            latencies = []  # Store per-DOI runtimes

            with st.spinner("Fetching from OpenAlex... please wait"):
                for idx, row in df.iterrows():
                    if pd.notna(row['DI']):
                        doi = str(row['DI']).strip()
                        start = time.time()
                        df.at[idx, 'OpenAlex_KW'] = fetch_openalex_keywords(doi)
                        end = time.time()
                        latencies.append({"DOI": doi, "Latency_sec": round(end - start, 3)})

            st.success("✅ Process completed!")

            # Show preview of first 5 rows
            st.dataframe(df[['DI', 'OpenAlex_KW']].head())

            # ---------------- Latency Analysis ----------------
            if latencies:
                lat_df = pd.DataFrame(latencies)
                median_latency = np.median(lat_df["Latency_sec"])
                q1 = np.percentile(lat_df["Latency_sec"], 25)
                q3 = np.percentile(lat_df["Latency_sec"], 75)

                st.markdown("### ⏱️ Latency Benchmarking")
                st.write(
                    f"**Median per-DOI latency:** {median_latency:.2f} seconds "
                    f"(IQR: {q1:.2f} – {q3:.2f} seconds)"
                )
                st.markdown(generate_csv_download(lat_df, filename="Latency_Log.csv"), unsafe_allow_html=True)

            # ---------------- Enriched Dataset ----------------
            st.markdown("### 📄 Download Enriched Dataset")
            st.markdown(
                "The enriched dataset includes a new `OpenAlex_KW` column. "
                "Choose your format below:"
            )
            st.markdown(generate_excel_download(df), unsafe_allow_html=True)
            st.markdown(generate_csv_download(df), unsafe_allow_html=True)

            # ---------------- Error Log (Failed DOIs) ----------------
            failed_df = df[df['OpenAlex_KW'].str.contains("ERROR", na=False)]
            if not failed_df.empty:
                st.warning(f"⚠️ {len(failed_df)} DOIs failed. A separate log is available for review.")
                st.markdown(
                    generate_csv_download(failed_df, filename="Failed_DOIs.csv"),
                    unsafe_allow_html=True
                )

            # ---------------- Warning Log (No Keywords) ----------------
            no_kw_df = df[df['OpenAlex_KW'].str.contains("NoKeywords", na=False)]
            if not no_kw_df.empty:
                st.info(f"ℹ️ {len(no_kw_df)} DOIs returned no keywords. A separate log is available.")
                st.markdown(
                    generate_csv_download(no_kw_df, filename="NoKeyword_DOIs.csv"),
                    unsafe_allow_html=True
                )
