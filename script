...
uploaded_file = st.file_uploader("📄 Upload Excel File (must contain 'Title', 'Author', 'Year', and 'DI' columns)", type=["xlsx"])

CONFIDENCE_THRESHOLD = 95

# --- Function to Extract DOI with compulsory author/year check ---
...
if uploaded_file:
    ...
    if not required_columns.issubset(df.columns):
        st.error("❌ The uploaded file must have 'Title', 'Author', and 'Year' columns.")
    else:
        if 'DI' not in df.columns:
            df['DI'] = pd.NA

        if st.button("🚀 Extract DOIs"):
            with st.spinner("Looking up missing DOIs using Crossref with strict matching..."):
